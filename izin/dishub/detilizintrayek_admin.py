...
class DetilTrayekAdmin(admin.ModelAdmin):
	...
	def view_pengajuan_trayek(self, request, id_pengajuan):
		...
		if pengajuan_:
			...
			extra_context.update({'pengajuan':pengajuan_, 'pemohon': pengajuan_.pemohon, 'perusahaan':pengajuan_.perusahaan, 'status': pengajuan_.status, 'kendaraan_list': kendaraan_list, 'kendaraan_count':kendaraan_count, 'skizin':skizin_, 'url_cetak': reverse("admin:trayek__cetak_skizin", kwargs={'id_pengajuan': pengajuan_.id}),})
	...
	def cetak_skizin(self, request, id_pengajuan):
		extra_context = {}
		pengajuan_obj = get_object_or_404(DetilTrayek, id=id_pengajuan)
		skizin_obj = pengajuan_obj.skizin_set.last()
		extra_context.update({
			'pengajuan' : pengajuan_obj,
			'skizin' : skizin_obj,
			'title' : "Cetak SK Izin Angkutan Trayek "+pengajuan_obj.get_no_skizin()
			})
		return render(request, "front-end/include/formulir_iua/cetak_skizin_trayek.html", extra_context)
	...
	def get_urls(self):
		...
		my_urls = patterns('',
			...
			url(r'^cetak/(?P<id_pengajuan>[0-9]+)$', self.admin_site.admin_view(self.cetak_skizin), name='trayek__cetak_skizin'),
			)
		return my_urls + urls
