	...
	if 'id_pengajuan' in request.COOKIES.keys():
		if request.COOKIES['id_pengajuan'] != '':
			...
			if pengajuan_.tanggal_imb_lama:
				...
				tanggal_imb_lama = ""
			if nama_bangunan != "":
				data = {'success': True,'data':{'nama_bangunan': nama_bangunan,'luas_bangunan': luas_bangunan,'jumlah_bangunan': jumlah_bangunan,'luas_tanah': luas_tanah,'status_tanah': status_tanah,'no_surat_tanah': no_surat_tanah,'tanggal_surat_tanah': tanggal_surat_tanah,'luas_bangunan_lama': luas_bangunan_lama,'no_imb_lama': no_imb_lama,'tanggal_imb_lama': tanggal_imb_lama,'id_kecamatan':id_kecamatan,'id_desa':id_desa}}
			else:
				id_kecamatan = ""
...
def load_identifikasi_bangunan_imb(request,id_pengajuan):
	if 'id_pengajuan' in request.COOKIES.keys():
		if request.COOKIES['id_pengajuan'] != '':
			pengajuan_ = DetilIMB.objects.get(pengajuanizin_ptr_id=request.COOKIES['id_pengajuan'])
			if pengajuan_.jenis_bangunan != None:
				if pengajuan_.jenis_bangunan.kode == "BK1":
