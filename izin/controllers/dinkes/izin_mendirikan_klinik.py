...
from izin_dinkes.models import MendirikanKlinik, JenisKlinik

def formulir_izin_mendirikan_klinik(request):
	extra_context={}
	if 'id_kelompok_izin' in request.COOKIES.keys():
		jenispermohonanizin_list = JenisPermohonanIzin.objects.filter(jenis_izin__id=request.COOKIES['id_kelompok_izin'])
		extra_context.update({'jenispermohonanizin_list': jenispermohonanizin_list})
		extra_context.update({'title': 'Izin Mendirikan Klinik'})
		...
		if 'id_pengajuan' in request.COOKIES.keys():
			if request.COOKIES.get('id_pengajuan', None) is not None and request.COOKIES.get('id_pengajuan') != '0':
				try:
					pengajuan_obj = MendirikanKlinik.objects.get(id=request.COOKIES.get('id_pengajuan'))
					extra_context.update({'pengajuan_': pengajuan_obj})
					extra_context.update({'pengajuan_id': pengajuan_obj.id})
				except ObjectDoesNotExist:
					pass
		template = loader.get_template("admin/izin/izin/dinkes/form_wizard_izin_mendirikan_klinik.html")
