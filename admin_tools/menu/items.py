...
class MenuItem(object):
    ...
    def is_selected(self, request):
        ...
        # current_path_func, myargs, mykwargs = resolve(request.path)
        # menu_url_func_name = '#'
        # if self.url and self.url != '#' and self.url != '':
        #     menu_url_func, myargs, mykwargs = resolve(self.url)
        #     menu_url_func_name = menu_url_func.__name__
        # return current_path_func.__name__ == menu_url_func_name or \
        #     len([c for c in self.children if c.is_selected(request)]) > 0

        current_url = request.get_full_path()
        current_url = current_url.split("?")
        return self.url == current_url[0] or \
            len([c for c in self.children if c.is_selected(request)]) > 0
